### Olá, sou Lucas Gabryel 👨🏻‍💻
#### Desenvolvedor de Software

Atualmente focado em arquitetura escalável e entrega de valor em sistemas de alta criticidade. Minha abordagem une rigor técnico na engenharia de software com uma visão estratégica voltada para resultados de negócio, aplicando esses fundamentos hoje no ecossistema de grandes empresas como a multinacional <a href="https://www.edenred.com.br/" target="_blank">Edenred</a>.

### 🛠️ Expertise Técnica

* **Engenharia de Backend:** Desenvolvimento de APIs robustas e escaláveis utilizando Java e Node.js, com foco em integração de sistemas e resiliência.
* **Desenvolvimento de Interfaces:** Criação de aplicações modernas e performáticas com React e Next.js, priorizando a experiência do usuário e eficiência de carregamento.
* **Qualidade e Padrões:** Aplicação rigorosa de princípios de Clean Code, arquitetura de software e versionamento estratégico para garantir a longevidade do código.

### 📈 Desempenho e Estatísticas

<p align="left">
  <img src="https://github-readme-stats.vercel.app/api?username=lucas-gabryel&show_icons=true&theme=cobalt&hide_border=true&title_color=fff&text_color=f3f3f3&icon_color=0077B5&locale=pt-br&hide_rank=true" height="165" />
  <img src="https://github-readme-stats.vercel.app/api/top-langs/?username=lucas-gabryel&layout=compact&theme=cobalt&hide_border=true&title_color=fff&text_color=f3f3f3&locale=pt-br" height="180" />
</p>

### 🤝 Contato para Projetos

Se você busca um desenvolvedor com experiência em ambientes de alta exigência para tirar seu projeto do papel ou escalar sua solução atual, entre em contato:

<a href="https://www.linkedin.com/in/lucas-gabryel/" target="_blank"><strong>Acessar perfil no LinkedIn</strong></a> 
&nbsp;&nbsp;|&nbsp;&nbsp; 
<a href="mailto:lucas-gabryel@outlook.com" target="_blank"><strong>Enviar e-mail profissional</strong></a>
